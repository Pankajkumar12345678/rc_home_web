        ...
        {isMenuOpen && (
          ...
          >
            <div className="flex flex-col space-y-1 p-4">
              {menuItems.map((item) => (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-4 py-3 font-medium transition-all ${
                    activeMenu === item.path ? 'scale-105' : ''
                  }`}
                  style={{ 
              ...
              <Link
                to="/contact"
                className="mt-6 flex items-center justify-center gap-2 rounded-full py-3 font-bold text-white transition-all hover:scale-105"
                style={{ 
                  background: `linear-gradient(135deg, ${colors.gradientStart} 0%, ${colors.gradientEnd} 100%)`,
                  boxShadow: `0 6px 20px rgba(${colors.primary}, 0.3)`
                }}
